...
#  @cases
#  Scenario:   通过权益id禁用权益----报错
#    Given 预置数据 操作类型delete 库名"cn_turboradio_module_test_equity" 表名"equity" 删除条件路径"src\test\resources\datapreset\equity-GuaranteeDisableEquityidDelete"
#    Given 预置数据 操作类型insert 库名"cn_turboradio_module_test_equity" 表名"equity" 入库数据路径"src\test\resources\datapreset\equity-GuaranteeDisableEquityidInsert"
#    Then 请求接口 接口地址"guarantee/disable/equityid" 参数文件路径"src\test\resources\senddata\equity-GuaranteeDisableEquityidBody"
#    Then 校验返回值 数据路径"src\test\resources\assertdata\equity-GuaranteeDisableEquityidResponse" 忽略字段""
